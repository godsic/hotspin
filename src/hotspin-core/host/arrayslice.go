...
func Array2D(size0, size1 int) ([]float64, [][]float64) {
	...
	sliced := make([][]float64, size0)
	list := make([]float64, size0*size1)
	for i := 0; i < size0; i++ {
...
func Array3D(size0, size1, size2 int) ([]float64, [][][]float64) {
	sliced := make([][][]float64, size0)
	for i := range sliced {
		sliced[i] = make([][]float64, size1)
	}
	list := make([]float64, size0*size1*size2)
	for i := range sliced {
...
func Array4D(size0, size1, size2, size3 int) ([]float64, [][][][]float64) {
	...
	for i := range sliced {
		for j := range sliced[i] {
			sliced[i][j] = make([][]float64, size2)
		}
	}
	list := make([]float64, size0*size1*size2*size3)
	for i := range sliced {
		for j := range sliced[i] {
...
func Slice4D(list []float64, size []int) [][][][]float64 {
	...
	for i := range sliced {
		for j := range sliced[i] {
			sliced[i][j] = make([][]float64, size[2])
		}
	}
...
func Array5D(size0, size1, size2, size3, size4 int) ([]float64, [][][][][]float64) {
	...
	for i := range sliced {
		for j := range sliced[i] {
			for k := range sliced[i][j] {
				sliced[i][j][k] = make([][]float64, size3)
			}
		}
	}
	list := make([]float64, size0*size1*size2*size3*size4)
	for i := range sliced {
		for j := range sliced[i] {
			for k := range sliced[i][j] {
