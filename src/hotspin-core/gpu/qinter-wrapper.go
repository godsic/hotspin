...
func Qinter_async(Qi *Array, Ti *Array, Tj *Array, Gij *Array, GijMul []float64, stream Stream) {
	...
	C.QinterAsync(
		(*C.float)(unsafe.Pointer((Qi.Comp[X].pointer))),

		(*C.float)(unsafe.Pointer((Ti.Comp[X].pointer))),
		(*C.float)(unsafe.Pointer((Tj.Comp[X].pointer))),

		(*C.float)(unsafe.Pointer((Gij.Comp[X].pointer))),
